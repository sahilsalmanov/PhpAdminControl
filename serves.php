...
if($_POST) {
    ...
    $sorgu = $connectToSql -> query("delete from xidmetlerimiz");
    $sorgu = $connectToSql -> query("insert into xidmetlerimiz (aciqlama) values ('$aciqlama')");
    if($sorgu) {
        echo "<script> window.location.href = 'serves.php';</script>";
    }
    else {
        echo "<script> alert('HATA: Kayit yapilamadi!');
        window.location.href = 'serves.php';
        </script>";
    }
...
$sorgu = $connectToSql -> query("select * from xidmetlerimiz");
